...
import landtitleFakerFactory from "./landtitleFakerFactory";
...
const LandtitleFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("titleowner")
                        .find({ query: { $select: ["_id","ownerId"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = landtitleFakerFactory(props.user,fakerCount,ownerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("landtitle").create(elem));
...
export default connect(mapState, mapDispatch)(LandtitleFakerDialogComponent);
