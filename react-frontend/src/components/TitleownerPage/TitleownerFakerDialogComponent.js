...
import titleownerFakerFactory from "./titleownerFakerFactory";
...
const TitleownerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = titleownerFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("titleowner").create(elem));
...
export default connect(mapState, mapDispatch)(TitleownerFakerDialogComponent);
